...
It emits the band-pass filtered signal (``band_samples``, one per-channel value
per sample): the filtered value for every sample the global window gained since
the pipeline last ran the processors (``state.samples_since_run``) — one sample
per tick at the realtime cadence, a batch when the global cadence is throttled.

Because the filter is applied to the global window from scratch each run, the
filtered value at a given sample is the same regardless of the run cadence (once
that sample has settled), so the rendered traces match across rates. ``latest``
also carries the per-channel RMS of the filtered window as a stable amplitude.
...
class BandSelectProcessor(EEGProcessor):
    ...
    def process(self, state: ProcessingState) -> dict:
        ...
        # so their filtered values are independent of how often process() runs.
        ...
        # Emit the filtered value for every sample the window gained since the
        # last run (one per tick at realtime, a batch when the run cadence is
        # throttled) — the values are identical, only the batching differs.
        m = min(state.samples_since_run, filtered.shape[0])
        out: dict = {"latest": rms}
        if m > 0:
